...
export class AnarchyRoll {
  ...
  async rollPool() {
    ...
    await this.subrolls.pool.evaluate();
  ...
  async rollRerollForced() {
    ...
    if (removed > 0) {
      this.subrolls.removed = new Roll(
        `-${removed}d1cf=1[${ROLL_THEME["removed"]}]`,
      );
      await this.subrolls.removed.evaluate();
  ...
  async rollGlitchDice() {
    if (this.param.glitch > 0) {
      ...
      this.subrolls.glitch = new Roll(
        `${this.param.glitch}d6cf=1[${ROLL_THEME["glitch"]}]`,
      );
      await this.subrolls.glitch.evaluate();
      this.subrolls.glitch.dice[0].options.appearance = {
  ...
  async rollAnarchyRisk() {
    if (this.param.risk > 0) {
      this.subrolls.risk = new Roll(
        `${this.param.risk}drcs>=5[${ROLL_THEME["anarchyRisk"]}]`,
      );
      await this.subrolls.risk.evaluate();
      this.subrolls.risk.dice[0].options.appearance = {
        colorset: RISK_COLORSET,
      };
      this.riskGlitch = this.subrolls.risk.terms[0].results.filter(
        (it) => it.result == 1,
      ).length;
      this.riskProwess += this.subrolls.risk.terms[0].results.filter(
        (it) => it.result >= 5,
      ).length;

...
import { BASE_MONITOR, TEMPLATE } from "../core/constants.js";
...
export class AnarchyBaseActor extends Actor {
  static init() {
    Hooks.on("updateActor", (actor, updates, options, id) => {
      const owner = AnarchyUsers.firstResponsible(actor);
      if (owner?.onUpdateActor) {
        owner.onUpdateActor(updates, options);
      }
    });
  }
  ...
  static sortSkills(actor, skills) {
    ...
    return skills.sort((skilla, skillb) => {
      const skillaIsKnowledge =
        skilla.system.code === "knowledge" ||
        skilla.system.attribute === "knowledge";
      const skillbIsKnowledge =
        skillb.system.code === "knowledge" ||
        skillb.system.attribute === "knowledge";
      ...
      const skillATotal =
        actor.getAttributeValue(skilla.system.attribute) + skilla.system.value;
      const skillBTotal =
        actor.getAttributeValue(skillb.system.attribute) + skillb.system.value;
  ...
  static sortAttributeButton(buttons) {
    ...
    return buttons.sort((a, b) => {
      if (game.i18n.localize(a.labelkey) > game.i18n.localize(b.labelkey))
        return 1;
      if (game.i18n.localize(a.labelkey) < game.i18n.localize(b.labelkey))
        return -1;
      return 0;
  ...
  getAllowedUsers(permission = CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER) {
    return game.users.filter((user) =>
      this.testUserPermission(user, permission),
    );
  }
  ...
  prepareDerivedData() {
    ...
    if (this.system.monitors) {
      Object.entries(this.system.monitors).forEach((kv) => {
        kv[1].maxBonus = Modifiers.sumMonitorModifiers(
          this.items,
          kv[0],
          "max",
        );
        kv[1].resistanceBonus = Modifiers.sumMonitorModifiers(
          this.items,
          kv[0],
          "resistance",
        );
  ...
  async onUpdateActor(updates, options) {
    if (
      updates.system?.monitors != undefined &&
      updates.system?.state == undefined
    ) {
  ...
  async defSetMatrixMonitor(path, value) {
    if (!this.getMatrixDetails().hasMatrix) {
      game.system.anarchy.hacks.i18n.format(
        ANARCHY.actor.monitors.noMatrixMonitor,
        {
          actor: this.name,
        },
      );
  ...
  _getMonitorMax(attribute) {
    ...
    return attributeValue == 0
      ? 0
      : BASE_MONITOR + Misc.divup(attributeValue, 2);
  }
  ...
  getUsableAttributes(item = undefined) {
    ...
    const usableAttributes = Misc.distinct(
      this.getAttributes().concat(itemsAttributes),
    );
    usableAttributes.sort(
      Misc.ascendingBySortedArray(Enums.sortedAttributeKeys),
    );
    ...
    if (attribute) {
      if (this.getAttributes().includes(attribute)) {
        ...
        if (candidateItems.length > 0) {
          const candidateValues = candidateItems.map(
            (it) => it.getAttributeValue(attribute) ?? 0,
          );
          value = Math.max(...candidateValues);
        }
      } else if (
        this.isEmerged() &&
        attribute == TEMPLATE.attributes.firewall
      ) {
  ...
  async rollWeapon(weapon) {
    ...
    const targeting = {
      attackerTokenId: game.scenes.current?.tokens.find(
        (it) => it.actor?.id == this.id,
      )?.id,
  ...
  async switchMonitorCheck(monitor, index, checked, sourceActorId = undefined) {
    await Checkbars.switchMonitorCheck(
      this,
      monitor,
      index,
      checked,
  ...
  async onEnterCombat() {
    const sceneAnarchy = Modifiers.sumModifiers(
      this.items,
      "other",
      "sceneAnarchy",
    );
  ...
  async spendCredibility(count) {
    await Checkbars.addCounter(
      this,
      TEMPLATE.counters.social.credibility,
      -count,
    );
  }
  ...
  getSkillValue(skillId, specialization = undefined) {
    ...
    return (
      skill.system.value +
      attribute +
      (specialization && skill.system.specialization ? 2 : 0)
    );
  }
  ...
  hasFavorite(type, id) {
    ...
    return this.system.favorites.find((it) =>
      AnarchyBaseActor._isSameFavorite(search, it),
    )
      ? true
      : false;
  }

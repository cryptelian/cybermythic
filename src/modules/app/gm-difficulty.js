...
export class GMDifficulty {
  ...
  async _rebuild() {
    this.toolbar
      .find(".gm-difficulty-bar")
      .replaceWith(await this._renderBar());
    this.toolbar.off("click", "a.click-roll-difficuty-pool");
    this.toolbar.on("click", "a.click-roll-difficuty-pool", async (event) => {
      event.preventDefault();
      await this._onClickDifficulty(event);
    });
  }

  async _renderBar() {
    return await renderTemplateSafe(
      templatePath("app", "gm-difficulty-buttons.hbs"),
      {

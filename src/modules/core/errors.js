...
export class ErrorManager {
    ...
    if (item.type != expectedType) {
      const error = game.i18n.format(ANARCHY.common.errors.expectedType, {
        type: game.i18n.localize(
          item.type ? ANARCHY.itemType.singular[item.type] : item.type,
        ),
  ...
  static checkActorCanReceiveDamage(damageType, monitor, actor) {
    if (!monitor) {
      const error = game.i18n.format(
        ANARCHY.common.errors.actorCannotReceiveDamage,
        {
          actor: actor.name,
  ...
  static checkTargetsCount(maxTargets, targets, area) {
    if (maxTargets > 0 && targets.length > maxTargets) {
      const error = game.i18n.format(
        ANARCHY.common.errors.maxTargetsExceedeed,
        {
          weapon: this.name,
  ...
  static checkActorDefenseAction(actorAction, actor, defense) {
    if (!actorAction) {
      const error = game.i18n.format(
        ANARCHY.common.errors.actorDoesNotHaveDefense,
        {
          actor: actor.name,

...
export class Enums {
  ...
  static init() {
    ...
    Enums.hbsShadowampCategories = Enums.mapObjetToKeyValue(
      ANARCHY.shadowampCategory,
    );
    Enums.hbsAreas = Enums.mapObjetToKeyValue(ANARCHY.area);
    Enums.hbsRanges = Enums.mapObjetToKeyValue(ANARCHY.range);
    Enums.hbsVehicleCategories = Enums.mapObjetToKeyValue(
      ANARCHY.vehicleCategory,
    );
  ...
  static getEnums(filterAttributes = (it) => true, withKnowledge = false) {
    return {
      ...
      skills: game.system.anarchy.skills
        .getSkills({ withKnowledge })
        .map((it) => {
          return {
            value: it.code,
            label: game.i18n.localize(it.labelkey),
